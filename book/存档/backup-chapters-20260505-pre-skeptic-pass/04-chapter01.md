...
第二章里你会看到一个真实的例子：同一批公开实验数据，只改一步处理，一个关键数字会从 **1.1** 跳到 **2.3**。那不是魔法，是在提醒：**读数和规则，必须摊开来说。**

书里如果出现「NIST」「Bootstrap」这类词，都只是**数据来源**和**算误差的一种方式**；不懂可以跳过名词，抓住主线——**假大象要经得起同一套测量工具的检验。**
