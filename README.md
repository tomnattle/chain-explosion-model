...
将（默认）生成 **`test_artifacts/figures/`** 归档图、**`test_artifacts/suite_report.json`**（机器可读）与 **`test_artifacts/README_TEST_REPORT.md`**，并**自动更新下方区块**（含各用例门禁结论、参数摘要图与 stdout 折叠日志）。加 `--no-artifacts` 可关闭归档；`--no-readme-patch` 只写 `test_artifacts` 不改本 README。

After a full suite run, figures land in **`test_artifacts/figures/`**, metrics in **`suite_report.json`**, and the section below is auto-patched into this README.
