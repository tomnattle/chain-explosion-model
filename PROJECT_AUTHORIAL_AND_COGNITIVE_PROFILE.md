...
- **功能：** 把长周期、多分支任务组织为**可执行的阶段结构**；降低叙事涣散风险。
...
- **核心做法：** 假设、指标公式、通过/不通过条件尽量**先固定在 JSON/Markdown**；事后改口需新版本文件（如 `ROUND2_HYPOTHESIS.md` 中的 Freeze Rule）。
...
| **控制感需求** | 长 README、统一套件、并行研究路线总览 | 通过**把不确定性装进 named scope** 来管理焦虑；Round 2 收束文是对「不确定性外溢」的补救文本类型。 |
| **强叙事冲动与强纠错机制并存** | 一侧是对抗性隐喻与宣言体；另一侧是「非声称」「不构成对量子非局域的肯定」 | 仓库内可见**张力**；成熟做法是把冲动性表述**回收**为指向 `ROUND2_CLOSURE_PAPER.md` 的指针（见现行 `ROUND2_FINAL_DECLARATION.md`）。 |
...
这一现象序列表明：写作目标从「推进单一研究线」迁移到「把研究线关进有边界的档案馆」。
...
- **隐喻成本：** 对抗性话语在团队外或跨界读者处可能触发**不信任**（像 advocacy 而非 research）；收束文档是对该成本的显式支付。
...
> **「预注册的对抗性工程写作」**：用版本化的规则与门禁代替一次性激情判断；用双语表格与 JSON 锚定语义；用多条「并行研究线」隔离不同类型结论；并在周期末用**闭包文档**明确可说的话和不可说的话，使仓库在法律与科学社交意义上可长期静置。
